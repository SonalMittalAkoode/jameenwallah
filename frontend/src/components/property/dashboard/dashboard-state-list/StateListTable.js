...
import { getAllStates, deleteState } from "@/api/state";
...
const StateListTable = () => {
  ...
  const fetchStates = useCallback(async () => {
    try {
      ...
      const response = await getAllStates(token, { includeInactive: true });
  ...
  }, [searchTerm, statusFilter, states, currentPage]);
  ...
  const handleDelete = async (stateId) => {
    ...
    try {
      ...
      await deleteState(stateId, token);
  ...
  if (error) {
    return (
      <div className="text-center py-4">
        ...
        <button className="ud-btn btn-thm mt-3" onClick={fetchStates}>
          Retry
        </button>
      </div>
    );
          ...
          {paginatedStates.map((state) => (
            <tr key={state._id}>
              <th scope="row" style={{ textAlign: "left" }}>
                <div className="h6 list-title">{state.name}</div>
              </th>
              ...
              <td className="vam" style={{ textAlign: "left" }}>
                <span
                  style={{
                    backgroundColor:
                      state.status === "active" ? "#28a745" : "#6c757d",
                    color: "#ffffff",
                    padding: "4px 12px",
                    borderRadius: "4px",
                    fontSize: "12px",
              ...
              <td className="vam" style={{ textAlign: "right" }}>
                <div className="d-flex justify-content-end gap-2">
                  <Link
                    href={`/cmsadminlogin/add-state?editId=${state._id}`}
                    className="icon"
                    style={{
                      border: "none",
                      background: "none",
                    ...
                    data-tooltip-id={`edit-${state._id}`}
                  >
                    <span
                  ...
                  <button
                    ...
                    data-tooltip-id={`delete-${state._id}`}
