...
import { getAllArchitects, deleteArchitect } from "@/api/architect";
...
const ArchitectListTable = () => {
  ...
  const fetchArchitects = useCallback(async () => {
    try {
      ...
      const response = await getAllArchitects(token);
      ...
      setError(err.response?.data?.message || "Failed to fetch architects");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const filtered = architects.filter((architect) => {
      const name = architect.name?.toLowerCase() || "";
      const email = architect.email?.toLowerCase() || "";
      const phone = architect.phoneNumber?.toLowerCase?.() || "";
      const status = architect.status?.toLowerCase() || "";
  ...
  }, [searchTerm, statusFilter, architects, currentPage]);
  ...
  const handleDelete = async (architectId) => {
    ...
    try {
      ...
      await deleteArchitect(architectId, token);
      alert("Architect deleted successfully!");
      fetchArchitects();
    } catch (err) {
      console.error("Error deleting architect:", err);
      alert(err.response?.data?.message || "Failed to delete architect");
    }
  };
  ...
  return (
    <>
      ...
      {filteredArchitects.length === 0 ? (
        ...
        >
          <p style={{ margin: 0 }}>No architects found.</p>
        </div>
      ) : (
            ...
            {paginatedArchitects.map((architect) => (
              <tr key={architect._id}>
                ...
                <td className="vam" style={{ textAlign: "left" }}>
                  <span
                    style={{
                      backgroundColor: architect.status === "active" ? "#28a745" : "#6c757d",
                      color: "#ffffff",
                      padding: "4px 12px",
                      borderRadius: "4px",
                      fontSize: "12px",
                ...
                <td className="vam" style={{ textAlign: "right" }}>
                  <div className="d-flex justify-content-end gap-2">
                    <Link
                      href={`/cmsadminlogin/add-architect?edit=${architect._id}`}
                      className="icon"
                      style={{ border: "none", background: "none", cursor: "pointer" }}
                      data-tooltip-id={`edit-${architect._id}`}
                    >
                    ...
                    <button
                      className="icon"
                      style={{ border: "none", background: "none", cursor: "pointer" }}
                      data-tooltip-id={`delete-${architect._id}`}
                    ...
                    <ReactTooltip
                      id={`edit-${architect._id}`}
                      place="top"
                      content="Edit"
                    />
                    <ReactTooltip
                      id={`delete-${architect._id}`}
                      place="top"
                      content="Delete"
                    />
                  </div>
...
export default ArchitectListTable;
