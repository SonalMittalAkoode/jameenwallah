...
import { getAllBlogCategories, deleteBlogCategory } from "@/api/blogCategory";
...
const BlogCategoryTable = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to fetch blog categories");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      ...
      await deleteBlogCategory(categoryId, token);
    ...
    <>
      <div className="dashboard_search_meta d-flex flex-column flex-md-row align-items-md-center justify-content-end gap-2 mb20">
        ...
        <div className="d-flex gap-2 align-items-center" style={{ maxWidth: "220px" }}>
          <label
            htmlFor="blogCategoryStatusFilter"
            ...
            <tr key={category._id}>
              ...
              <td className="vam" style={{ textAlign: "right" }}>
                <div className="d-flex justify-content-end gap-2">
                  <Link
                    href={`/cmsadminlogin/add-blog-category?editId=${category._id}`}
                    className="icon"
