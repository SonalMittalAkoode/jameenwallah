...
import { getAllTestimonials, deleteTestimonial } from "@/api/testimonial";
...
const TestimonialListTable = () => {
  ...
  const fetchTestimonials = useCallback(async () => {
    try {
      ...
      const response = await getAllTestimonials(token);
      ...
      if (response.status === "success" && response.data) {
        ...
        const pages = Math.max(
          1,
          Math.ceil(response.data.length / ITEMS_PER_PAGE)
        );
        setTotalPages(pages);
  ...
  useEffect(() => {
    const filtered = testimonials.filter((testimonial) => {
      ...
      const status = testimonial.status?.toLowerCase() || "";
  ...
  }, [searchTerm, statusFilter, testimonials, currentPage]);
  ...
  if (error) {
    return (
      <div className="text-center py-4">
        ...
        <button className="ud-btn btn-thm mt-3" onClick={fetchTestimonials}>
          Retry
        </button>
      </div>
    );
  ...
  return (
    <>
      <div className="dashboard_search_meta d-flex flex-column flex-md-row align-items-md-center justify-content-end gap-2 mb20">
        ...
        >
          <label
            htmlFor="testimonialStatusFilter"
              ...
              {paginatedTestimonials.map((testimonial) => (
                <tr key={testimonial._id}>
                  ...
                  <td style={{ textAlign: "left" }}>
                    <div className="list-title">{testimonial.name || "N/A"}</div>
                  </td>
                  ...
                  <td className="vam" style={{ textAlign: "center" }}>
                    <span
                      style={{
                        backgroundColor:
                          testimonial.status === "active" ? "#28a745" : "#6c757d",
                        color: "#ffffff",
                        padding: "4px 12px",
                        borderRadius: "4px",
                        fontSize: "12px",
                  ...
                  <td className="vam" style={{ textAlign: "right" }}>
                    <div className="d-flex justify-content-end gap-2">
                      <Link
                        href={`/cmsadminlogin/add-testimonial?editId=${testimonial._id}`}
                        className="icon"
                        style={{
                          border: "none",
                          background: "none",
                        ...
                        data-tooltip-id={`edit-${testimonial._id}`}
                      >
                        <span
                      ...
                      <button
                        ...
                        data-tooltip-id={`delete-${testimonial._id}`}
