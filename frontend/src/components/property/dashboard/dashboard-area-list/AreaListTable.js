...
const AreaListTable = () => {
  ...
  const fetchAreas = useCallback(async () => {
    try {
      ...
      const response = await getAllAreas(token);
  ...
  useEffect(() => {
    const filtered = areas.filter((area) => {
      ...
      const cityName = area.city?.name?.toLowerCase() || "";
  ...
  }, [searchTerm, statusFilter, areas, currentPage]);
  ...
  if (error) {
    return (
      <div className="text-center py-4">
        ...
        <button className="ud-btn btn-thm mt-3" onClick={fetchAreas}>
          Retry
        </button>
      </div>
    );
          ...
          {paginatedAreas.map((area) => (
            <tr key={area._id}>
              <th scope="row" style={{ textAlign: "left" }}>
                <div className="h6 list-title">{area.name}</div>
              </th>
              <td style={{ textAlign: "left" }}>
                <div className="list-title">{area.city?.name || "N/A"}</div>
              </td>
              ...
              <td className="vam" style={{ textAlign: "left" }}>
                <span
                  style={{
                    backgroundColor:
                      area.status === "active" ? "#28a745" : "#6c757d",
                    color: "#ffffff",
                    padding: "4px 12px",
                    borderRadius: "4px",
                    fontSize: "12px",
              ...
              <td className="vam" style={{ textAlign: "left" }}>
                <span
                  style={{
                    backgroundColor:
                      area.isTrending === "active" ? "#FC9401" : "#6c757d",
                    color: "#ffffff",
                    padding: "4px 12px",
                    borderRadius: "4px",
                    fontSize: "12px",
                ...
                >
                  {area.isTrending || "Deactive"}
                </span>
              </td>
              <td className="vam" style={{ textAlign: "right" }}>
                <div className="d-flex justify-content-end gap-2">
                  <Link
                    href={`/cmsadminlogin/add-area?editId=${area._id}`}
                    className="icon"
                    style={{
                      border: "none",
                      background: "none",
                  ...
                  <button
                    ...
                    data-tooltip-id={`delete-${area._id}`}
